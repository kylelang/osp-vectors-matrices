...
::: {.callout-note}
Running matrix arithmetic is Apply; explaining *why* recycling behaves the way
it does with matrices (a direct extension of what you learned with vectors) is Understand.
:::
...
- Why does `m1 + v1` recycle silently, while `m1 + v2` recycles with a warning? What's the underlying rule?
- When replacing matrix elements, why does R accept a replacement vector whose length "cleanly divides" the
  selection length, but reject other length mismatches?
